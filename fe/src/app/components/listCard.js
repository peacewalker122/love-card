	...
	return (
		...
		>
			{val.map((item, index) => (
				<div
					key={`div-${index}`}
					className={`card bg-white border border-gray-200 rounded-lg shadow-md p-4 w-96 m-4 flex-shrink-0 transform transition-transform duration-500 ${
						expandedCard !== null ? "hidden" : ""
					}`}
